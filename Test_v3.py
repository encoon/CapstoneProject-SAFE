...
def update_plot(frame):
    ...
    axs[0].plot(COB_graph, label="ICOB", color="blue")
    axs[0].set_title("Center of Balance Transition (ICOB)")
    axs[0].set_ylim(0, 400) 
    axs[0].legend()
    axs[0].grid(False)  # Disable grid for faster rendering

    axs[1].plot(WSD_graph, label="ME", color="orange")
    axs[1].set_title("Muscular Efficiency (ME)")
    ...
    axs[3].plot(EMG_graph, label="Average Difference", color="yellow")
    axs[3].set_title("Average Difference of Fatigue Coefficients")
            ...
            if (len(COB_Arr) < 30):
                ...
                if (len(FC_Arr) > 5):
                    ...
                    for n in range(1,6):
                        ...
                        last_FC = FC_Arr[n]
                    if (sum(AVG_Diff) / len(AVG_Diff) < 70):
                        print("Fatigued Muscle")
                        GPIO.output(4, GPIO.HIGH)
                    else:
                        GPIO.output(4, GPIO.LOW)
                        
                    EMG_graph.append(sum(AVG_Diff)/len(AVG_Diff))

                    AVG_Diff = []
                    FC_Arr = []

ani = FuncAnimation(fig, update_plot, interval=1000, blit=False)  # Update every 2 seconds with blit enabled
